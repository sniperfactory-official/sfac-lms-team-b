...
const useSelectCourse = ({courseList, setCurrentCourse}: IArg) => {
  ...
  const [selectedCourse, setSelectedCourse] = useState(Array.from({length:courseList.length}, (_,idx) => idx === 0 ? true : false))
  // 클릭 시, 현재 선택한 Course데이터 state에 저장
  const handleCurrentCourse = ({course, idx}: {course:ICourseField, idx:number}) => {
  ...
  useEffect(() => {
    setSelectedCourse(Array.from({length:courseList.length}, (_,idx) => idx === 0 ? true : false))
  }, [courseList])
