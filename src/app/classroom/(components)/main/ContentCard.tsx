...
const ContentCard = ({ lecture }: { lecture: ILecture }) => {
  ...
  console.log(lecture);
  const {title,  lectureType, isPrivate, startDate, endDate, lectureContent} = lecture
  ...
  return (
    <div className="w-[775px] h-[200px] border rounded-lg p-[10px] flex flex-row items-center mb-[15px]">
      ...
      <div className="w-2/3 h-5/6 ml-20px flex flex-col">
        ...
        {lectureType === "비디오" && <div className="bg-grayscale-5 rounded w-[40px] h-[20px] text-xs text-center leading-[20px] mb-[10px] text-grayscale-60">
          {LECTURE_OBJ[lectureType].time}</div>}
        <div className="font-bold mb-[10px]">{LECTURE_OBJ[lectureType].emoji} {lecture.title}</div>
